  ...
  return (
    <section id="workshops" className="relative py-24 px-4">
      <div className="max-w-6xl mx-auto">
        {/* Header — text from left, image from right, meeting in the middle */}
        <div className="grid md:grid-cols-2 gap-8 items-center mb-16">
          ...
          <ScrollMotion from="right" blur distance={140}>
        ...
        {/* Workshop grid — cards cascade in from alternating sides */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {WORKSHOPS.map((w, i) => (
            <ScrollMotion key={w.name} from={i % 2 ? 'right' : 'left'} distance={70} delay={Math.min(i * 0.04, 0.2)} className="h-full">
              <Tilt max={14} scale={1.06} className="h-full">
                <article
                  className="group glass-panel border border-cyan/15 hover:border-lime/40 p-5 transition-all cursor-pointer relative overflow-hidden h-full">
